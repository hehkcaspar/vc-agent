...
_PROMPT = (
    ...
    "2. Is the source category correct? Classify the ITEM itself by "
    "its MEDIUM, not by what the item is about. Use the 'click test': "
    "if the authoritative URL opens a(n) ...\n"
    ...
    "3. If the source category is WRONG, pick the category the item "
    "actually belongs to from the list above (news | patents | startups "
    "| red_flags | papers). Leave empty only if nothing in that list "
    "fits.\n"
    "4. Identify the single authoritative URL (the primary / official "
...
async def verify_item(
    ...
    *,
    ...
    return VerifyResult(
        verdict=(parsed.get("verdict") or "").strip().lower() or "unconfirmed",
        authoritative_url=(
            grounding_urls[0]
            if grounding_urls

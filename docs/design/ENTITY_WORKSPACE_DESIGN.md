...
workspace_read_file(path)                            # read file: PDFs as compressed native binary (Gemini) or extracted text (Kimi); Office/text as extracted text
...
Set by:
- **Metadata preprocess job** — `run_metadata_preprocess_job` extracts `one_liner` via Gemini and promotes it to `metadata_json.description` (only if not already set, so manual annotations take precedence). This runs during Process Inbox background extraction (Path B) and standalone `POST .../metadata-preprocess` triggers.
- **Process Inbox Path A** — per-file extraction in Pass 1 also promotes `one_liner` → `description` inline.
- **Chat agent** — after deep-reading a file, uses `workspace_annotate(path, description)` to set/update description.
- **User** — inline edit on file card in UI.

Tool: `workspace_annotate(path, description)` — sets description directly, overriding any auto-extracted value.
...
| `services/agent_harness.py` | ReAct agent harness (replaced old portfolio_deep_agent.py). Workspace tools only, no SDK built-ins |

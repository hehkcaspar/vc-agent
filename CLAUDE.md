...
- `components/EntityDetail.tsx` — shell with horizontal tab row (**Workroom** | **Facts**, `role="tablist"`). Workroom (default) is the existing workspace tree + chat split; Facts renders `<EntityFactsTab>`. Header (`<EntityHeader>`) + discrepancy panel + edit modal sit above the tab row. `useEntity` for fresh `last_content_at`.
...
- **Layer 2 — Source fetchers** (`sources/`): semantic_scholar_papers, google_scholar_stats, patents_lens, news_web, crunchbase_startups, red_flags_watch. Each owns one external API. Layer 3 dim agents call `trigger_refresh` only — never external APIs directly. **Snapshot contract**: every `run()` must call `record_snapshot()` on ALL exit paths (incl. early returns), or heartbeat fires the source every tick.

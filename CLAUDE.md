...
- **`models.py`** — SQLAlchemy ORM (entities, workspace_nodes, workspace_ops, ingest_items, conversation_sessions/messages, chat_completion_jobs). `entities.metadata_json` TEXT column holds extracted VC metadata (populated by `extract_info` preset, synced from `Company Profile.json`).

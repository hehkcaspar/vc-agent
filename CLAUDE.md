...
- `origin_type` (upload|agent|ingest|shared|user) gates writes — agents cannot overwrite `upload`/`ingest`/**`user`** files (the `user` protection is what makes the inline `.md`/`.json` editor's auto-flip stick: a save flips `origin_type → user` so subsequent agent runs raise `ProtectedFileError` instead of clobbering the edit). `shared` (e.g., `WORKSPACE_NOTES.md`) stays agent-writable.
...
- **Upload modes** (`FileUploadModal` in `EntityDetail.tsx`): Files / Folder / Zip / Text. Files + Folder route through the **signed-URL flow** (`POST /workspace/upload-init` → XHR PUT direct to object store → `POST /workspace/upload-commit`) so bytes bypass Cloud Run's 32 MB request-body ceiling on prod. Zip mode still uses the legacy `POST /workspace/upload-zip` (server unpack with size cap + zip-slip rejection). Text mode wraps pasted content as `note-<ISO>.md`. The adapter (`services/storage.py::StorageAdapter.mint_upload`) picks the strategy: `GcsSignedUrlAdapter` on Cloud Run (`GCS_BUCKET` set) issues v4 signed PUT URLs via IAM SignBlob; `LocalFilesystemAdapter` returns `use_direct_upload=True` so local dev falls back to the legacy `POST /workspace/file` endpoint transparently. Frontend XHR PUT (`frontend/src/services/upload.ts`) surfaces `loaded/total` natively for a progress bar with rate + ETA. Signed-URL setup: bucket CORS policy at `backend/gcs-cors.json` + compute SA's `roles/iam.serviceAccountTokenCreator` self-binding + `GCS_OBJECT_PREFIX=entities/` env var matching the FUSE mount layout.
...
- `main.tsx` — mount point. Opts into React Router v7 future flags (`v7_startTransition`, `v7_relativeSplatPath`) and applies `data-theme` from localStorage synchronously before React renders so pre-Layout surfaces (the ViewportGuard denial) paint in the correct theme. Wraps `<BrowserRouter>` with `<ViewportGuard>`.
- `components/ViewportGuard.tsx` — posture gate. Uses `window.matchMedia('(min-width: 768px)')` to decide whether to render the full app or a polite denial screen (brand `<h1>`, "designed for screens 768px and wider", live width readout). Reactive — resizing up from phone width into ≥768 swaps to the real app without a reload. The tool is desktop-first; below 768 the denial is the entire page. Mobile-drawer CSS / `.mobile-toggle` / `.mobile-header` JSX in `Layout.tsx` is dead code behind the guard — left in place for possible later un-guarding.
- `components/Layout.tsx` — sidebar (Portfolio / Academic / Settings; theme toggle in footer). Auto-collapses to icon rail at 768 ≤ width ≤ 1024. Sidebar brand is a `<div>` (not an `<h1>`) so each page's content heading is the page's only `<h1>`. Nav items carry `title` + `aria-current="page"`; mobile toggle carries `aria-label` / `aria-expanded` / `aria-controls`.
- `components/PortfolioTab.tsx` — portfolio list/grid view. Entity rows and cards are semantic `<Link to="/portfolio/entities/{id}">` anchors (not `<div onClick>`), so middle-click / right-click / Tab focus / screen readers all work; nested action buttons `e.preventDefault()` to cancel the anchor when clicked. Toolbar splits into two independent filter dims + view toggle. **Stage** (workflow, default `funnel` = prospect+diligence+portfolio): `Funnel | Prospect | Diligence | Portfolio | Passed | Exited`. No "All stages" button — it was visually redundant with Funnel on small portfolios. **Status** (archival, default `active`): `Active | Archived | All`. Filters combine via AND; counts on each segment reflect the other dim's full pool. Sessions that stored the legacy `stageFilter='active'` or `'all'` migrate to `'funnel'` on load (`normaliseStageFilter`).
...
- `components/EntityDetail.tsx` — shell with horizontal tab row (`role="tablist"`): **Workroom** | **Facts** | **Initial Screening** (appears when `Deliverables/Memos/initial_screening.md` exists) | **Initial Screening v2** (appears when `..._v2.md` exists) | **News** (when tracking enabled). When both screening memos exist the labels disambiguate as **Screening v1** / **Screening v2**; when only one exists its label collapses to **Initial Screening**. Active screening tab falls back to Workroom if its memo disappears from the tree. Workroom (default) is the existing workspace tree + chat split. Header (`<EntityHeader>`) + discrepancy panel + edit modal sit above the tab row. `useEntity` for fresh `last_content_at`. Tablist implements W3C APG roving tabindex: only the active tab has `tabIndex=0`, the rest are `-1`; `ArrowLeft` / `ArrowRight` / `Home` / `End` cycle focus, with focus restored via a post-commit effect keyed off `pendingTabFocusRef` (so click-to-select doesn't steal focus from nested surfaces).
...
- `components/EntityInitialScreeningTab.tsx` — path-agnostic editable view of a Taihill Monday-Screening memo. Props: `memoPath`, optional `reviewPath`. Splits the md on h2 boundaries and renders each section as a `.facts-section` card using `ReactMarkdown` + `remark-gfm` inside `.markdown-viewer` (chrome parity with Facts tab). Meta bar shows generated-at (from `GET /workspace/node/{id}` → `updated_at`) + a **Source** button (routes `onOpenPreview` back to Workroom so the file preview panel is visible) + a **Review notes** disclosure that lazy-loads the sibling `*_review_notes.md` on first open + a **Recompose** button (re-runs the composer phase against existing section JSONs, ~10 s). **Per-section inline edit**: every section card has a `Pencil` button next to its `<h3>` that swaps the body to a `<textarea>` with Save / Cancel; Save reassembles the full memo via `reassembleScreening({prologue, sections})` (the parser captures pre-h2 prologue lines so the `# {entity}` h1 round-trips losslessly) and PUTs to `/workspace/file/{nid}/content` with `cache: 'no-store'` plus immediate local `setMdContent` to defeat HTTP cache + refetch races. First save flips `origin_type → user`, blocking subsequent agent overwrites. Follow-up-questions sections get a `.facts-section--followup` gold left-accent; the review card gets a muted `.facts-section--review` wash. Exports `hasScreeningMemo(tree, path)` so the parent can gate tab visibility. Works with either v1's older schema (`Why it matters / Team (facts only) / …`) or the current Taihill template (`Intro / [1] Team / … / [6] Source / Follow-up`) — the parser is purely structural.
...
- `components/EntityEditModal.tsx` — full canonical-fact editor. Sections: **Identity & deal** (name, one-liner, description, business model, HQ location, industry tags, referral source, deal stage radios), **Founders** (name + role + status active/departed editable rows; add/remove), **Key team** (name + role rows), **Team size** (current_headcount + target_headcount), **Our positions** (positions list with fund dropdown — inline `+ Add new fund…` POSTs `/settings/funds`). Clones `entity.metadata` before overrides so extract_info fields are preserved. Pairs with `EntityInitialScreeningTab.tsx`'s **Recompose** button: edit facts here → close → click Recompose to regenerate the memo against the new canonical state.
...
- `components/academic/` — Academic Tracking v2 UI. `AcademicTab` toolbar is **List | Ranking** (the old "Tasks" tab was consolidated into Settings > Continuous Tasks; legacy `/academic?view=tasks` URLs redirect to `/settings/academic-tasks` via `useEffect` + `navigate(..., { replace: true })`). `ScholarDetail` header renders the scholar name as `<h1>` so every scholar tab has a single page heading; `NarrativeReportView` in the Report tab demotes its own headline to `<h2>` and sub-sections to `<h3>` for proper hierarchy. Other pieces: `RankingView` (table uses `min-width: 720px` on `.ranking-table` + `scrollbar-width: thin` + webkit scrollbar styling on `.ranking-table-wrapper` so the dim columns stay reachable behind a visible horizontal scrollbar at tablet widths), `AddScholarModal`, `EditProfileModal`, `TasksView` with `ct-*` class prefix (only mounted inside `Settings/sections/TasksSettings.tsx`), etc.
...
- `styles/primitives.css` — single source of truth for `.modal*`, `.btn-*`, `.form-*`, `.radio-group`, `.tag-menu*`. Imported once from `main.tsx`. **Do not redefine these in component CSS** — add component-scoped classes instead. Includes a global `:focus-visible` gold-accent ring (2 px, offset 2 px) applied via `:where(a, button, input, select, textarea, [role="tab"], ...)` so keyboard focus is visible everywhere without clobbering mouse clicks.
- `components/ui/Modal.tsx` — the only modal primitive. Handles overlay + Esc + scroll-lock + header + **focus trap**: Tab / Shift+Tab cycle within the modal, initial focus lands on the first non-close focusable (or an autoFocus-requested child) after paint, focus returns to the trigger element on close. `size` prop: `narrow` | `standard` | `wide`. Children render directly inside `.modal` so `<form>` can wrap body+footer.
...
### Responsive posture
Desktop-first. **`<768px` → polite denial screen** (`ViewportGuard`); **`≥768px` → full app**. Mobile breakpoints throughout the CSS (`Layout.css`, `PortfolioTab.css`, `AcademicTab.css`, `Settings.css`) use `max-width: 767px` so viewports of exactly 768px (iPad portrait) land in the collapsed-sidebar tablet layout, not mobile-drawer. The `.mobile-toggle` / `.mobile-header` JSX and corresponding CSS are unreachable behind the guard — kept in place pending a later cleanup pass so a future un-guard doesn't regress.
...
### File preview
`FilePreview` in `EntityDetail.tsx` — side panel + expand-to-popup modal (lucide `Maximize2`). Markdown via `react-markdown` + `remark-gfm`. Popup: filename + version-picker dropdown (text/markdown only, lazy `GET /workspace/file/{id}/versions`, blob via `/{version}`) + copy-to-clipboard + **Edit** (lucide `Pencil`, only on text-shaped current versions: `.md` / `.markdown` / `.json` / `.txt` / `.csv`; never on uploaded versions or binary files) + close. Edit swaps the body to a `<textarea>` with Save / Cancel replacing Copy + version-picker; for `.json` files Save validates `JSON.parse` client-side first and shows an inline error if invalid. Save PUTs to `/workspace/file/{nid}/content` (auto-flips `origin_type → user` so subsequent agent runs raise `ProtectedFileError`), then `setContent(editedText)` immediately and bumps refetch — defeats HTTP cache + refetch races. Versions are view-only; side panel always shows current. `downloadFile` / `downloadFileVersion` / `downloadFileByPath` use `cache: 'no-store'` so post-save reads see the new bytes.
...
- Config: `data/config/{funds.json, legal_templates.json, legal_review_checklist.json, continuous_tasks_overrides.json, dimensions.json, heartbeat.json, field_archetypes.json, ranking_presets/, digests/}` — `continuous_tasks_overrides.json` stores only user DELTAS (sparse); effective config = `deep_merge(seed, overrides)` computed in memory on every read.
...
- **Layer 2 — Source fetchers** (`sources/`): google_scholar_papers, semantic_scholar_papers, google_scholar_stats, patents_web, news_web, startups_web, red_flags_watch. Each owns one external API (or grounded-search target). `news_web`, `patents_web`, and `startups_web` share the Gemini grounded-search pattern via `llm_client.grounded_search_json` + a structured relevance filter; `startups_web` populates `startups.json`, which `news_web._collect_known_ventures` then reads to enrich the news query. Layer 3 dim agents call `trigger_refresh` only — never external APIs directly.
  - **`papers.json` is co-written by two primary sources**: `google_scholar_papers` (SerpAPI `google_scholar_author` with direct HTML-scrape fallback, sort=pubdate — delivers recency) and `semantic_scholar_papers` (SS API — enriches matched rows with `authorId`, `external_ids.DOI`, `s2_fields`, `influential_citations` and appends long-tail older papers GS hasn't surfaced). Both call `papers_merge._merge_papers_by_priority(...)` which encodes the 3-way priority matrix (GS / SS / stub). `google_scholar_papers` uses incremental early-break pagination (`known_gs_ids=prev_gs_ids`) so steady-state refreshes collapse to 1 API call. SS's refactored `run()` no longer wholesale-rewrites papers.json. Rows carry a `_source` marker plus `_was_ss` / `_was_stub` audit flags.
  - **Routing + verification pipeline**: `item_verification.verify_item()` runs one `gemini-3.1-flash-lite-preview` grounded search per item to check claim + category correctness; `item_triage.triage()` returns `keep | drop | route` (pure); on `route`, `destinations.accept_into()` hands off to the destination ledger (only `papers` is implemented — appends a stub when SS hasn't yet indexed a paper routed from another source); on `drop`, a tombstone is appended to `_tombstones.jsonl` which the next prompt reads as a "do not re-emit" block. Orchestrated by `refinement.refine_pending_items()` — fired fire-and-forget from each source after persist. URL quality guaranteed by `url_fallback.apply_url_fallback()` (3-tier: grounding → HEAD-validated LLM → `google.com/search`).
  - **Paper attribution under heuristic position**: `attributed_metrics.compute_attributed_metrics()` prefers `authorId == scholar_ss_id` match, falls back to the row-level `_author_position` (set by `google_scholar_papers._infer_position()` via surname/initial match on GS's comma-separated authors string, with "sole" / "first" / "last" / "middle" cases and ellipsis-truncation handling). Papers where both fail are counted in `position_unknown_papers` for observability. `PaperResponse` (router) carries `source`, `is_stub`, `was_ss`, `was_stub` so the frontend `PublicationsTab` can badge row provenance.
...
All cadences/models/required_sources come from the **effective config** — `deep_merge(seed, overrides)` computed in-memory on every read. Seed: tracked `backend/app/services/academic/continuous_tasks_seed.json` (ships in the backend image via `COPY backend/`). Overrides: sparse user deltas in `data/config/continuous_tasks_overrides.json`, mutated only via `PATCH /academic/continuous-tasks/...` through `continuous_overrides.set_override(...)`. `continuous_config.load_continuous_tasks()` is a pure read + merge; no write-on-read side effects. Legacy full-copy `continuous_tasks.json` files are one-shot migrated to overrides on first load (renamed with `.pre-overrides.bak` suffix). `heartbeat.py` dispatches per tick. Module-level `get_heartbeat_status()` exposes liveness.
...
- **Seed + overrides** (`continuous_config.py` + `continuous_overrides.py`): seed is read-only canonical structure+defaults; overrides is a sparse deltas-only file (`continuous_tasks_overrides.json`). Effective config = pure `deep_merge(seed, overrides)` on every read — no write-on-read. Deep-merge rules: scalars replaced; nested dicts recursed; lists replaced wholesale. Seed deprecations propagate automatically (dropped from effective config on next read).
- `GET /academic/continuous-tasks` — catalog + per-task health (7d runs, success rate, avg duration, last error) + heartbeat liveness.
- `PATCH /academic/continuous-tasks/{kind}/{task_id}` — edit `enabled` / `default_cadence_days` / `priority_overrides`. Dry-run merges the pending override with the seed and `validate_merged(...)` runs the Pydantic schema + cross-ref check BEFORE touching disk; bad patches never persist. Writes only the delta via `set_override(...)`; atomic tmp-write + rename. Audit entry to eval log.
...
- Frontend tasks view: **Settings > Continuous Tasks** (reached via the Settings sidebar or `/settings/academic-tasks`). Renders `TasksView` with inline cadence edit, enable toggle, run-now, expandable detail. Legacy `/academic?view=tasks` URLs redirect here. The Academic tab toolbar is **List | Ranking** only.
